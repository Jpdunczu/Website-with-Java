...
<html>
    ...
    function transmit(){
        var trans = setInterval( function(){rowA(getFrame);},10000 );
        counter(frame);
    }
    
    function counter(frame){
        var timeout = setTimeout(function(){ time(frame);},10000);
    }
    ...
    function rowA(frame){
        if( frame.pBit !== 0 ){
            ...
            if( frame.ack >0 && frame.ack === rowAtFrames[((rowAtFrames.length) -1)].ack){
    ...
    function rowAtransmitFrame(frame){
        ...
        //setTimeout(function(){ rowB(frame); },4000);
    }
    ...
    function rowB(frame){
        ...
        if( frame.pBit === 1 ){
            ...
            if( frame.sqcNum <= 7 ) {
                ...
                frame.ack += 1;
                setFrame(frame);
                setTimeout(function(){rowA(frame);}, 4000);
                setTimeout(document.getElementById("success2").innerHTML = "Acknowledging frame <b>" + frame.ack + "</b>",4000);
            }
        }
    }
